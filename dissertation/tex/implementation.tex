...
Running the model checker is computationally expensive, especially when
the path length in the bounded model checker is high or when there is a
high branching factor in the event graph being checked. For example,
model checking a small, correct example from the lock usage test suite
with a length bound of 2000 instructions takes around 2 minutes on my
development machine.

However, there are a number of opportunities for optimisation (both to
the code and to the algorithms as they are implemented) that could
potentially speed up the model checker. From a practical perspective,
there is a degree of futility in these optimisations as the model
checker is unlikely to ever be fast enough to run at compile time
(except perhaps on very small programs with few assertions).
