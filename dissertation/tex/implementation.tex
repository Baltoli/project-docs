...
TESLA allows for properties of data flow to be asserted---in particular,
the value of arguments passed to functions and the value returned by a
function call. Checking arguments passed to functions can be performed
by adapting existing TESLA code, and so in this section I describe only
the algorithm used to check function return values.
...
LLVM semantics would be far more complex.} For example, the LLVM
...
starting at the entry block. The blocks taken in the sequence constrain the
...
% TODO: improve counterexample generation and add output here
\begin{listing}
