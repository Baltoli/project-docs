...
maximum possible performance of each server. \autoref{fig:locks-bench} compares
the throughput for each implementation.
...
The server with runtime TESLA instrumentation achieves only 60\% throughput when
...
number of instructions executed in the TESLA runtime library is approximately
proportional to the throughput performance decreases observed. While this is an
imprecise analysis (running Callgrind on a program affects performance
significantly), it gives an indication that the overhead observed in the
statically analysed server is due to imperfections in the optimisation process
rather than the API wrapper functions.
