...
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}
    \node [sourcefile] (source1) {\texttt{.c}};

    \node [sourcefile, right=3em of source1] (obj1) {\texttt{.o}};
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \node [sourcefile, above=1em of mid] (bc1) {\texttt{.bc}};
    \node [sourcefile, below=1em of mid] (tesla1) {\texttt{.tesla}};

    \draw[-latex] (source1.east) to [out=0,in=180] (bc1.west);
    \draw[-latex] (source1.east) to [out=0,in=180] (tesla1.west);

    \node [sourcefile,right=5em of mid] (bc2) {\texttt{.bc}};

    \draw[-latex] (bc1.east) to [out=0,in=180] (bc2.west);
    \draw[-latex] (tesla1.east) to [out=0,in=180] (bc2.west);

    \node [right=3em of bc2] (exe) {executable};

    \draw[-latex] (bc2.east) -- (exe.west);
  \end{tikzpicture}
